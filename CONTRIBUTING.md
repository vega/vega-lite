...
If you send a pull request from a fork, the formatting, schema, and compiled Vega spec checks run automatically with no setup required. If you also want CI to regenerate the example images (SVG/PNG) on your branch, see [Examples](#examples).
...
The compiled Vega specs (`examples/compiled/*.vg.json`) and normalized specs (`examples/specs/normalized/*.vl.json`) are deterministic and are verified on every pull request, so they must be up to date. The rendered SVG and PNG images are produced by Vega's renderer and differ slightly across systems (mainly due to floating point differences), so they are **not** verified and should be regenerated by CI rather than committed from your machine.

There are two ways to keep examples up to date on a pull request from a fork:

1. **Let CI do it (recommended).** Enable GitHub Actions on your fork and add a [`GH_PAT` secret](https://docs.github.com/en/free-pro-team@latest/actions/reference/encrypted-secrets) for a [personal access token](https://docs.github.com/en/free-pro-team@latest/github/authenticating-to-github/creating-a-personal-access-token) with public repository permissions. CI then runs `npm run build:examples` on every push and commits the regenerated specs and images back to your branch, so you don't have to commit anything yourself.
2. **Commit the specs yourself.** Otherwise, run `npm run build:examples` locally and commit the updated `*.vg.json` and normalized `*.vl.json` files. Do **not** commit the SVG/PNG images—a maintainer or CI on the base repository will regenerate them.

You can run `npm run build:example <examplename>` (e.g., `npm run build:example bar_1d`) to rebuild a single example locally, and you're encouraged to run `npm run build:examples` to make sure your code does not cause unnecessary changes.
...
All pull requests will be tested on [GitHub Actions](https://github.com/features/actions). If your PR does not pass the checks, your PR will not be approved. The CI will run `npm test`, verify that the compiled Vega specs are up to date (see [Examples](#examples)), and check code coverage of your code. If you don't want your PR reviewed until checks pass, mark the [pull request as draft](https://help.github.com/en/github/collaborating-with-issues-and-pull-requests/about-pull-requests#draft-pull-requests). Once you're ready for review, convert the pull request to mark it as ready for review.
