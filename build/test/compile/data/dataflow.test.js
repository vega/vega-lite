    ...
    describe('DataFlowNode', function () {
        ...
        describe('remove', function () {
            ...
            it('should maintain order', function () {
                var root = new dataflow_1.DataFlowNode(null, 'root');
                var rootChild1 = new dataflow_1.DataFlowNode(root, 'rootChild1');
                var node = new dataflow_1.DataFlowNode(root, 'node');
                var rootChild2 = new dataflow_1.DataFlowNode(root, 'rootChild2');
                var child1 = new dataflow_1.DataFlowNode(node, 'child1');
                var child2 = new dataflow_1.DataFlowNode(node, 'child2');
                expect(root.children).toEqual([rootChild1, node, rootChild2]);
                chai_1.assert.equal(rootChild1.parent, root);
                chai_1.assert.equal(rootChild2.parent, root);
                chai_1.assert.equal(node.parent, root);
                chai_1.assert.equal(child1.parent, node);
                chai_1.assert.equal(child2.parent, node);
                node.remove();
                expect(root.children).toEqual([rootChild1, child1, child2, rootChild2]);
                chai_1.assert.equal(rootChild1.parent, root);
                chai_1.assert.equal(rootChild2.parent, root);
                chai_1.assert.equal(child1.parent, root);
                chai_1.assert.equal(child2.parent, root);
            });
        });
