...
import { assembleHeaderGroups, assembleHeaderProperties, assembleTitleGroup, defaultHeaderGuideAlign, defaultHeaderGuideBaseline, getLayoutTitleBand } from '../../../src/compile/header/assemble';
...
describe('compile/header/index', () => {
    ...
    describe('getTitleGroup', () => {
        ...
        describe('for column', () => {
            ...
            const { title } = columnLabelGroup, columnTitleGroupTopLevelProps = tslib_1.__rest(columnLabelGroup, ["title"]);
        ...
        describe('for row', () => {
            ...
            const { title } = rowTitleGroup, rowTitleGroupTopLevelProps = tslib_1.__rest(rowTitleGroup, ["title"]);
            it('returns a header group mark with correct name, role, and type.', () => {
    ...
    describe('assembleHeaderProperties', () => {
        describe('for title properties', () => {
            ...
            const headerTitleProps = assembleHeaderProperties(config, facetFieldDef, 'column', HEADER_TITLE_PROPERTIES, HEADER_TITLE_PROPERTIES_MAP);
            it('should return the correct title property from header', () => {
                expect(headerTitleProps.fontSize).toEqual(40);
            });
            const configTitleProps = assembleHeaderProperties(config, undefined, 'column', HEADER_TITLE_PROPERTIES, HEADER_TITLE_PROPERTIES_MAP);
            it('should return the correct title property from config', () => {
                expect(configTitleProps).toEqual({ fontSize: 20, offset: 10 });
            });
            const bothTitleProps = assembleHeaderProperties(config, facetFieldDef, 'column', HEADER_TITLE_PROPERTIES, HEADER_TITLE_PROPERTIES_MAP);
        ...
        describe('for label properties', () => {
            ...
            const config = model.config;
            const facetFieldDef = model.component.layoutHeaders['row'].facetFieldDef;
            const headerLabelProps = assembleHeaderProperties(config, facetFieldDef, 'column', HEADER_LABEL_PROPERTIES, HEADER_LABEL_PROPERTIES_MAP);
            it('should return the correct label property from header', () => {
                expect(headerLabelProps.fontSize).toEqual(40);
            });
            const configLabelProps = assembleHeaderProperties(config, undefined, 'column', HEADER_LABEL_PROPERTIES, HEADER_LABEL_PROPERTIES_MAP);
            it('should return the correct label property from config', () => {
                expect(configLabelProps).toEqual({ fontSize: 20, offset: 10 });
            });
            const bothLabelProps = assembleHeaderProperties(config, facetFieldDef, 'column', HEADER_LABEL_PROPERTIES, HEADER_LABEL_PROPERTIES_MAP);
