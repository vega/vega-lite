...
describe('compile/scale/assemble', () => {
    ...
    describe('assembleScaleRange', () => {
        ...
        it('updates width signal when renamed.', () => {
            ...
            expect(assembleScaleRange([0, SignalRefWrapper.fromName(model.getSignalName.bind(model), 'width')], 'x', 'x')).toMatchObject([0, { signal: 'new_width' }]);
        });
        it('updates height signal when renamed.', () => {
            ...
            expect(assembleScaleRange([0, SignalRefWrapper.fromName(model.getSignalName.bind(model), 'height')], 'x', 'x')).toMatchObject([0, { signal: 'new_height' }]);
        });
    });
});
