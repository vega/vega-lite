...
describe('compile/scale', () => {
    ...
    describe('padding', () => {
        ...
        it('should be continuousBandSize for linear x-scale of vertical bar.', () => {
            expect(rules.padding('x', 'linear', {}, { field: 'date', type: 'temporal' }, { type: 'bar', orient: 'vertical' }, { continuousBandSize: 13 })).toEqual(13);
        });
        it('should be undefined for linear x-scale for binned field of vertical bar.', () => {
            expect(rules.padding('x', 'linear', {}, { bin: true, field: 'date', type: 'temporal' }, { type: 'bar', orient: 'vertical' }, { continuousBandSize: 13 })).toEqual(undefined);
        });
        it('should be continuousBandSize for linear y-scale of horizontal bar.', () => {
            expect(rules.padding('y', 'linear', {}, { field: 'date', type: 'temporal' }, { type: 'bar', orient: 'horizontal' }, { continuousBandSize: 13 })).toEqual(13);
        });
    });
    ...
    describe('zero', () => {
        it('should return true when mapping a quantitative field to x with scale.domain = "unaggregated"', () => {
            expect(rules.zero('x', { field: 'a', type: 'quantitative' }, 'unaggregated', { type: 'point' }, 'linear')).toBeTruthy();
        });
        ...
        it('should return true when mapping a non-binned quantitative field to x/y of point', () => {
            for (const channel of ['x', 'y']) {
                expect(rules.zero(channel, { field: 'a', type: 'quantitative' }, undefined, { type: 'point' }, 'linear')).toBeTruthy();
            }
        });
        it('should return false when mapping a quantitative field to dimension axis of bar, line, and area', () => {
            for (const mark of [BAR, AREA, LINE]) {
                expect(rules.zero('x', { field: 'a', type: 'quantitative' }, undefined, { type: mark, orient: 'vertical' }, 'linear')).toBe(false);
                expect(rules.zero('y', { field: 'a', type: 'quantitative' }, undefined, { type: mark, orient: 'horizontal' }, 'linear')).toBe(false);
            }
        });
        it('should return false when mapping a binned quantitative field to x/y', () => {
            for (const channel of ['x', 'y']) {
                expect(!rules.zero(channel, { bin: true, field: 'a', type: 'quantitative' }, undefined, { type: 'point' }, 'linear')).toBeTruthy();
            }
        });
        it('should return false when mapping a non-binned quantitative field with custom domain to x/y', () => {
            for (const channel of ['x', 'y']) {
                expect(!rules.zero(channel, {
                    bin: true,
                    field: 'a',
                    type: 'quantitative'
                }, [3, 5], { type: 'point' }, 'linear')).toBeTruthy();
            }
        });
    });
});
