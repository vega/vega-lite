...
describe('compile/scale', function () {
    describe('type()', function () {
        it('should return null for channel without scale', function () {
            assert.deepEqual(scaleType({}, 'detail', { type: 'temporal', timeUnit: 'yearmonth' }, 'point', defaultScaleConfig), null);
        });
        it('should show warning if users try to override the scale and use bin', log.wrap(function (localLogger) {
            assert.deepEqual(scaleType({ type: 'point' }, 'color', { type: 'quantitative', bin: true }, 'point', defaultScaleConfig), ScaleType.BIN_ORDINAL);
        ...
        describe('nominal/ordinal', function () {
            describe('color', function () {
                it('should return ordinal scale for nominal data by default.', function () {
                    assert.equal(scaleType({}, 'color', { type: 'nominal' }, 'point', defaultScaleConfig), ScaleType.ORDINAL);
                });
            ...
            describe('discrete channel (shape)', function () {
                it('should return ordinal for nominal field', function () {
                    assert.deepEqual(scaleType({}, 'shape', { type: 'nominal' }, 'point', defaultScaleConfig), ScaleType.ORDINAL);
                });
                it('should return ordinal even if other type is specified', log.wrap(function (localLogger) {
                    [ScaleType.LINEAR, ScaleType.BAND, ScaleType.POINT].forEach(function (badScaleType) {
                        assert.deepEqual(scaleType({ type: badScaleType }, 'shape', { type: 'nominal' }, 'point', defaultScaleConfig), ScaleType.ORDINAL);
                ...
                it('should return ordinal for an ordinal field and throw a warning.', log.wrap(function (localLogger) {
                    assert.deepEqual(scaleType({}, 'shape', { type: 'ordinal' }, 'point', defaultScaleConfig), ScaleType.ORDINAL);
            ...
            describe('continuous', function () {
                    ...
                    PRIMITIVE_MARKS.forEach(function (mark) {
                        ...
                        [ORDINAL, NOMINAL].forEach(function (t) {
                            [X, Y].forEach(function (channel) {
                                assert.equal(scaleType({}, channel, { type: t }, mark, defaultScaleConfig), ScaleType.POINT);
                            });
                        });
                    });
                });
                    ...
                    [ORDINAL, NOMINAL].forEach(function (t) {
                        [X, Y].forEach(function (channel) {
                            ['bar', 'rule', 'rect'].forEach(function (mark) {
                                assert.equal(scaleType({}, channel, { type: t }, 'rect', defaultScaleConfig), ScaleType.BAND);
                            });
                        });
                    });
                });
                it('should return point scale for X,Y when mark is point', function () {
                    [ORDINAL, NOMINAL].forEach(function (t) {
                        [X, Y].forEach(function (channel) {
                            assert.equal(scaleType({}, channel, { type: t }, 'point', defaultScaleConfig), ScaleType.POINT);
                        });
                    });
                });
                it('should return point scale for X,Y when mark is point when ORDINAL SCALE TYPE is specified and throw warning', log.wrap(function (localLogger) {
                    [ORDINAL, NOMINAL].forEach(function (t) {
                        [X, Y].forEach(function (channel) {
                            assert.equal(scaleType({ type: 'ordinal' }, channel, { type: t }, 'point', defaultScaleConfig), ScaleType.POINT);
                            var warns = localLogger.warns;
                    ...
                    PRIMITIVE_MARKS.forEach(function (mark) {
                        [ORDINAL, NOMINAL].forEach(function (t) {
                            OTHER_CONTINUOUS_CHANNELS.forEach(function (channel) {
                                assert.equal(scaleType({}, channel, { type: t }, mark, defaultScaleConfig), ScaleType.POINT, channel + ", " + mark + ", " + t + " " + scaleType({}, channel, { type: t }, mark, defaultScaleConfig));
                            });
                        });
                    });
                });
        ...
        describe('temporal', function () {
            it('should return sequential scale for temporal color field by default.', function () {
                assert.equal(scaleType({}, 'color', { type: 'temporal' }, 'point', defaultScaleConfig), ScaleType.SEQUENTIAL);
            });
            it('should return ordinal for temporal field and throw a warning.', log.wrap(function (localLogger) {
                assert.deepEqual(scaleType({}, 'shape', { type: 'temporal', timeUnit: 'yearmonth' }, 'point', defaultScaleConfig), ScaleType.ORDINAL);
            ...
            it('should return time for all time units.', function () {
                for (var _i = 0, TIMEUNITS_1 = TIMEUNITS; _i < TIMEUNITS_1.length; _i++) {
                    ...
                    assert.deepEqual(scaleType({}, Y, { type: 'temporal', timeUnit: timeUnit }, 'point', defaultScaleConfig), ScaleType.TIME);
                }
            });
        });
        describe('quantitative', function () {
            it('should return sequential scale for quantitative color field by default.', function () {
                assert.equal(scaleType({}, 'color', { type: 'quantitative' }, 'point', defaultScaleConfig), ScaleType.SEQUENTIAL);
            });
            it('should return linear scale for piecewise quantitative color field by default.', function () {
                assert.equal(scaleType({ domain: [1, 2, 3], range: ['red', 'green', 'blue'] }, 'color', { type: 'quantitative' }, 'point', defaultScaleConfig), ScaleType.LINEAR);
            });
            it('should return ordinal bin scale for quantitative color field with binning.', function () {
                assert.equal(scaleType({}, 'color', { type: 'quantitative', bin: true }, 'point', defaultScaleConfig), ScaleType.BIN_ORDINAL);
            });
            it('should return ordinal for encoding quantitative field with a discrete channel and throw a warning.', log.wrap(function (localLogger) {
                assert.deepEqual(scaleType({}, 'shape', { type: 'quantitative' }, 'point', defaultScaleConfig), ScaleType.ORDINAL);
            ...
            it('should return linear scale for quantitative by default.', function () {
                assert.equal(scaleType({}, 'x', { type: 'quantitative' }, 'point', defaultScaleConfig), ScaleType.LINEAR);
            });
            it('should return bin linear scale for quantitative by default.', function () {
                assert.equal(scaleType({}, 'opacity', { type: 'quantitative', bin: true }, 'point', defaultScaleConfig), ScaleType.BIN_LINEAR);
            });
            it('should return linear scale for quantitative x and y.', function () {
                assert.equal(scaleType({}, 'x', { type: 'quantitative', bin: true }, 'point', defaultScaleConfig), ScaleType.LINEAR);
            });
        });
        describe('dataTypeMatchScaleType()', function () {
            it('should return specified value if datatype is ordinal or nominal and specified scale type is the ordinal or nominal', function () {
                assert.equal(scaleType({ type: ScaleType.ORDINAL }, 'shape', { type: 'ordinal' }, 'point', defaultScaleConfig), ScaleType.ORDINAL);
            });
            it('should return default scale type if data type is temporal but specified scale type is not time or utc', function () {
                assert.equal(scaleType({ type: ScaleType.LINEAR }, 'x', { type: 'temporal', timeUnit: 'year' }, 'point', defaultScaleConfig), ScaleType.TIME);
                assert.equal(scaleType({ type: ScaleType.LINEAR }, 'color', { type: 'temporal', timeUnit: 'year' }, 'point', defaultScaleConfig), ScaleType.SEQUENTIAL);
            });
            it('should return time if data type is temporal but specified scale type is discrete', function () {
                assert.equal(scaleType({ type: ScaleType.POINT }, 'x', { type: 'temporal', timeUnit: 'year' }, 'point', defaultScaleConfig), ScaleType.TIME);
            });
            ...
            it('should return default scale type if data type is quantative but scale type do not support quantative', function () {
                assert.equal(scaleType({ type: ScaleType.TIME }, 'color', { type: 'quantitative' }, 'point', defaultScaleConfig), ScaleType.SEQUENTIAL);
            });
            it('should return default scale type if data type is quantative and scale type supports quantative', function () {
                assert.equal(scaleType({ type: ScaleType.TIME }, 'x', { type: 'quantitative' }, 'point', defaultScaleConfig), ScaleType.LINEAR);
            });
            it('should return default scale type if data type is quantative and scale type supports quantative', function () {
                assert.equal(scaleType({ type: ScaleType.TIME }, 'x', { type: 'temporal' }, 'point', defaultScaleConfig), ScaleType.TIME);
            });
        });
    });
});
