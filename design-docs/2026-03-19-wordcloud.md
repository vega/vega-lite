  ...
  "marks": [
    {
      ...
      "encode": {
        "update": {
          "text": {"signal": "isValid(datum[\"word\"]) ? datum[\"word\"] : \"\"+datum[\"word\"]"},
          "align": {"value": "center"},
...
- **`rotate`** ← `angle` encoding (per-datum via field) or mark def `angle` (constant; default: random [-45, 0, 45])
...
The `angle` encoding field is passed to the wordcloud transform's `rotate` parameter so each word is rotated individually. When no `angle` encoding is specified, words are randomly rotated among -45, 0, and 45 degrees. To force a constant rotation, set `angle` on the mark def: `"mark": {"type": "wordcloud", "angle": 0}` (horizontal) or `"mark": {"type": "wordcloud", "angle": -45}`.
...
For `angle`, the raw field values are passed to the transform's `rotate` parameter. When no `angle` encoding or mark def `angle` is specified, the compiler emits a default random rotation expression (`~~(random() * 3) * 45 - 45`) that picks from [-45, 0, 45] per word — matching Vega's wordcloud example. Setting `angle: 0` on the mark def produces a horizontal-only layout.
