...
vg.data.force.dependencies = ["links"];vg.data.formula = (function() {
  ...
  return function() {
    ...
    var formula = vg.data.mapper(function(d, i, list) {
      if (field){
        var split = field.split("."), o=d;
        for(var i=0 ; i<split.length ; i++){
          if(i<split.length-1){
            o[split[i]] = o[split[i]] || {};
            o = o[split[i]];
          }else{
            o[split[i]] = expr.call(null, d, i, list);
          }
        }
      }
      return d;
...
vg.parse.properties = (function() {
  ...
  function ordinal(def, scale, rng, db, data) {
    ...
    // width-defined range
    if (def.bandWidth) {
      var space = def.points ?
        (def.padding||0) * def.bandWidth :
        def.bandWidth * (def.padding || 0) * (domain.length-1) + def.outerPadding *2;

      rng = [
        rng[0] || 0, // why range[0]?
        def.bandWidth * domain.length + space
      ];
    }
    ...
    if (str || rng.length > 2 || rng.length===1 || dataDrivenRange) {
      ...
      scale.rangeRoundBands(rng, def.padding || 0, def.outerPadding || def.padding || 0);
    } else {
      scale.rangeBands(rng, def.padding || 0, def.outerPadding || def.padding || 0);
    }
  }
