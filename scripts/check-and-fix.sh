...
git config --global user.email "ci@ci.org"
...
then
  ...
  git commit -m "[CI] Update schema (build: $BUILD_DEFINITIONNAME)"
fi
...
then
  git commit -m "[CI] Update examples (build: $BUILD_DEFINITIONNAME)"
fi
...
then
  ...
  git commit -m "[CI] Auto-formatting (build: $BUILD_DEFINITIONNAME)"
fi
