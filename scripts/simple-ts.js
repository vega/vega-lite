// modified from https://github.com/GoogleChromeLabs/proxx/blob/f54224c6aa6de7d5a6011136f9228db2b49ea858/lib/simple-ts.js
...
function loadConfig(searchPath, configName) {
  const fileName = ts.findConfigFile(searchPath, ts.sys.fileExists, configName);
...
export default function simpleTS(searchPath, configName, {noBuild, watch} = {}) {
  const config = loadConfig(searchPath, configName);

  const args = ['-p', join(searchPath, configName)];
  ...
  return {
    ...
    load(id) {
      ...
      const newId = join(config.options.outDir, relative(process.cwd(), id)).replace(extRe, '.js');
