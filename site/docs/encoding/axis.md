...
An axis configuration supports all [axis properties](#properties) except `position`, `orient`, `format`, `values`, `zindex`, and `title` (as in Vega, a specific title cannot be set via the config; however, you can set `"title": null` to disable axis titles by default). In addition, it also supports the `disable` property:
