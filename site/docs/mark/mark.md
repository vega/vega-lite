...
In addition to primitive marks, Vega-Lite also support composite marks, which are "macros" for complex layered graphics that contain multiple primitive marks. Supported composite mark types include [`"boxplot"`](boxplot.html), [`"density"`](density-mark.html), [`"errorband"`](errorband.html), [`"errorbar"`](errorbar.html).
