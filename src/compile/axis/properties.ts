...
function _defaultLabelAlign(angle: number, axisOrient: AxisOrient, startAngle: 0 | 90, mainOrient: 'bottom' | 'right') {
...
export function defaultLabelAlign(angle: number, axisOrient: AxisOrient, channel?: 'x' | 'y'): Align {
  ...
  if (angle !== undefined) {
    if (channel === 'x') {
      ...
      return _defaultLabelAlign(angle, axisOrient, 90, 'right');
    }
  }
  return undefined;
}
