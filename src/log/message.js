...
import {getSizeChannel} from '../channel.js';
...
export function selectionAsScaleDomainWithoutField(field) {
  return (
    'A "field" or "encoding" must be specified when using a selection as a scale domain. ' +
    `Using "field": ${stringValue(field)}.`
  );
}
export function selectionAsScaleDomainWrongEncodings(encodings, encoding, extent, field) {
  return (
    `${!encodings.length ? 'No ' : 'Multiple '}matching ${stringValue(encoding)} encoding found for selection ${stringValue(extent.param)}. ` +
    `Using "field": ${stringValue(field)}.`
  );
}
// CONCAT / REPEAT
