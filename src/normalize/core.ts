    ...
    for (const repeatValue of repeatValues) {
      for (const rowValue of row) {
        for (const columnValue of column) {
          ...
          // TODO: remove repeat_ prefix
          child.name =
            (repeatValue ? `repeat_repeat_${varName(repeatValue)}` : '') +
            (rowValue ? `repeat_row_${varName(rowValue)}` : '') +
            (columnValue ? `repeat_column_${varName(columnValue)}` : '');
