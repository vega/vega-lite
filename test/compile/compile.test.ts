...
describe('compile/compile', () => {
  ...
  it('should not reference independent lane scales from a shared area thickness scale', () => {
    const {spec} = compile({
      data: {values: [{value: 1, density: 0.5, group: 'A'}]},
      layer: [
        {
    ...
    const sizeScale = spec.scales.find((scale) => scale.name === 'size');
    const sizeRange = sizeScale && 'range' in sizeScale ? sizeScale.range : undefined;
    expect(JSON.stringify(sizeRange)).not.toContain('layer_');
    expect(() => parse(spec)).not.toThrow();
  });

  it('should not reference facet-local lane scales from a shared area thickness scale', () => {
    const {spec} = compile({
      data: {values: [{value: 1, density: 0.5, group: 'A', facet: 'one'}]},
      ...
      resolve: {scale: {y: 'independent', size: 'shared'}},
    });

    const sizeScale = spec.scales.find((scale) => scale.name === 'size');
    const sizeRange = sizeScale && 'range' in sizeScale ? sizeScale.range : undefined;
    expect(JSON.stringify(sizeRange)).not.toContain('child_y');
    expect(() => parse(spec)).not.toThrow();
  });
