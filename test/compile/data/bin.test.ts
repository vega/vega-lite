  ...
  describe('assembleFromEncoding', () => {
    ...
    it('should apply the bin transform only once for a binned field encoded in multiple channels', () => {
      ...
      expect(transform[2]).toEqual({
        ...
        expr: `datum["bin_maxbins_10_Rotten_Tomatoes_Rating"] === null || !isFinite(datum["bin_maxbins_10_Rotten_Tomatoes_Rating"]) ? "null" : format(datum["bin_maxbins_10_Rotten_Tomatoes_Rating"], "") + " - " + format(datum["bin_maxbins_10_Rotten_Tomatoes_Rating_end"], "")`
      });
    });
  });
