...
import {COLOR, DETAIL, FILLOPACITY, OPACITY, SIZE, STROKEOPACITY, UNIT_CHANNELS} from '../../../src/channel';
...
describe('Mark', () => {
  ...
  describe('pathGroupingFields()', () => {
    it('should return fields for unaggregate detail, color, size, opacity fieldDefs.', () => {
      for (const channel of [DETAIL, COLOR, SIZE, OPACITY, FILLOPACITY, STROKEOPACITY]) {
    ...
    it('should return condition detail fields for color, size, shape', () => {
      for (const channel of [COLOR, SIZE, OPACITY, FILLOPACITY, STROKEOPACITY]) {
        expect(
