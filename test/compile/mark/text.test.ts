  ...
  describe('with binned quantitative', () => {
    ...
    it('should output correct bin range', () => {
      expect(props.text).toEqual({
        signal: `datum["bin_maxbins_10_foo"] === null || !isFinite(datum["bin_maxbins_10_foo"]) ? "null" : format(datum["bin_maxbins_10_foo"], "d") + " - " + format(datum["bin_maxbins_10_foo_end"], "d")`
      });
    });
  });
  ...
  describe('with row, column, text, color, and size', () => {
    ...
    it('should map color to fill', () => {
      expect(props.fill).toEqual([
        {
          test: 'datum["mean_Acceleration"] === null || !isFinite(datum["mean_Acceleration"])',
          value: null
        },
        {
          scale: 'color',
