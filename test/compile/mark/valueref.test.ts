  ...
  describe('tooltipForEncoding', () => {
    it('returns correct tooltip signal for binning field', () => {
      expect(
        ...
        signal:
          '{"IMDB_Rating (binned)": datum["bin_maxbins_10_IMDB_Rating"] === null || !isFinite(datum["bin_maxbins_10_IMDB_Rating"]) ? "null" : format(datum["bin_maxbins_10_IMDB_Rating"], "") + " - " + format(datum["bin_maxbins_10_IMDB_Rating_end"], "")}'
      });
    });
    ...
    it('returns correct tooltip signal for binning field', () => {
      expect(
        ...
        signal:
          '{"IMDB_Rating (binned)": datum["bin_IMDB_rating"] === null || !isFinite(datum["bin_IMDB_rating"]) ? "null" : format(datum["bin_IMDB_rating"], "") + " - " + format(datum["bin_IMDB_rating_end"], "")}'
      });
    });
  });
});
