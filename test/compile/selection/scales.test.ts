    ...
    it('is assembled from selection parameter', () => {
      const model = parseConcatModel({
        vconcat: [
          ...
          {
            selections: [{name: 'brush3', select: {type: 'interval', fields: ['symbol']}}],
            mark: 'area',
            encoding: {
              x: {
                field: 'date',
      ...
      hconcat: [
        {
          ...
          selections: [{name: 'selector001', select: 'interval', bind: 'scales'}]
        },
        {
          mark: 'point',
          encoding: {
          ...
          selections: [{name: 'selector001', select: {type: 'interval'}, bind: 'scales'}]
        }
      ]
    });
