...
describe('Segment Selection on Explicit Segments', () => {
  ...
  it('builds tuple signals', () => {
    ...
    expect(signals).toContainEqual({
      ...
      on: [
        {
          events: [{signal: 'seg_x || seg_y'}],
          update: 'seg_x && seg_y ? {unit: "", values: [seg_x[0], seg_y[0], seg_x[1], seg_y[1]]} : null',
        },
      ],
    });
  });
  ...
  it('builds a visible segment mark', () => {
    ...
    expect(marks).toContainEqual({
      ...
      encode: {
        enter: {},
        update: {
          x: [{test: 'data("seg_store").length && data("seg_store")[0].unit === ""', signal: 'seg_x_1[0]'}, {value: 0}],
          y: [{test: 'data("seg_store").length && data("seg_store")[0].unit === ""', signal: 'seg_y_1[0]'}, {value: 0}],
          x2: [
            {test: 'data("seg_store").length && data("seg_store")[0].unit === ""', signal: 'seg_x_1[1]'},
            {value: 0},
          ],
          y2: [
            {test: 'data("seg_store").length && data("seg_store")[0].unit === ""', signal: 'seg_y_1[1]'},
            {value: 0},
          ],
          stroke: {value: 'white'},
          strokeWidth: {value: 2},
  ...
  it('replaces the current segment on redraw and clears visual extents', () => {
    ...
    expect(signals.find((signal) => signal.name === 'seg_x_1')?.on).toEqual(
  ...
  it('generates a segment intersection predicate', () => {
    expect(parseSelectionPredicate(model, {param: 'seg'})).toBe(
      '!length(data("seg_store")) || (max(min(data("seg_store")[0].values[0], data("seg_store")[0].values[2]), min(+(datum["x1"]), +(datum["x2"]))) <= min(max(data("seg_store")[0].values[0], data("seg_store")[0].values[2]), max(+(datum["x1"]), +(datum["x2"]))) && max(min(data("seg_store")[0].values[1], data("seg_store")[0].values[3]), min(+(datum["y1"]), +(datum["y2"]))) <= min(max(data("seg_store")[0].values[1], data("seg_store")[0].values[3]), max(+(datum["y1"]), +(datum["y2"])))) && (((+(datum["x2"]) - +(datum["x1"])) * (data("seg_store")[0].values[1] - +(datum["y1"])) - (+(datum["y2"]) - +(datum["y1"])) * (data("seg_store")[0].values[0] - +(datum["x1"])))) * (((+(datum["x2"]) - +(datum["x1"])) * (data("seg_store")[0].values[3] - +(datum["y1"])) - (+(datum["y2"]) - +(datum["y1"])) * (data("seg_store")[0].values[2] - +(datum["x1"])))) <= 0 && ((((data("seg_store")[0].values[2] - data("seg_store")[0].values[0]) * (+(datum["y1"]) - data("seg_store")[0].values[1]) - (data("seg_store")[0].values[3] - data("seg_store")[0].values[1]) * (+(datum["x1"]) - data("seg_store")[0].values[0])))) * ((((data("seg_store")[0].values[2] - data("seg_store")[0].values[0]) * (+(datum["y2"]) - data("seg_store")[0].values[1]) - (data("seg_store")[0].values[3] - data("seg_store")[0].values[1]) * (+(datum["x2"]) - data("seg_store")[0].values[0])))) <= 0',
    );
  });
});
  ...
  it('builds tuple signals from intersected series tuples', () => {
    ...
    expect(signals).toContainEqual({
      ...
      on: [
        {
          events: [{signal: 'seg_x || seg_y'}],
          update: 'seg_x && seg_y ? data("seg_tuples") : []',
        },
      ],
    });
  });
