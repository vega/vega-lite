...
describe('vl.field.cardinality()', function () {
  describe('for Q', function () {
    it('should return cardinality', function() {
      ...
      var cardinality = vlfield.cardinality(encDef, stats);
...
describe('vl.field.isType', function () {
  it('should return correct type checking', function() {
    ...
    expect(vlfield.isType(qDef, Q)).to.eql(true);
    expect(vlfield.isTypes(qDef, N)).to.eql(false);
  });
});
...
describe('vl.field.isTypes', function () {
  it('should return correct type checking', function() {
    var qDef = {name: 'number', type:'Q'};
    expect(vlfield.isType(qDef, Q)).to.eql(true);
    expect(vlfield.isTypes(qDef, [Q])).to.eql(true);
    expect(vlfield.isTypes(qDef, [Q, O])).to.eql(true);
    expect(vlfield.isTypes(qDef, [O, Q])).to.eql(true);
    expect(vlfield.isTypes(qDef, [Q, N])).to.eql(true);
    expect(vlfield.isTypes(qDef, [N])).to.eql(false);
  });
});