...
import {normalizeAutoSizes} from '../src/toplevelprops';
...
export function parseUnitModel(spec: TopLevel<UnitSpec>) {
  return new UnitModel(spec, null, '', undefined, undefined, initConfig(spec.config), normalizeAutoSizes(spec.autosize, spec.config ? spec.config.autosize : undefined, true).type === 'fit');
}
...
export function parseLayerModel(spec: TopLevel<LayerSpec>) {
  return new LayerModel(spec, null, '', undefined, undefined, initConfig(spec.config), normalizeAutoSizes(spec.autosize, spec.config ? spec.config.autosize : undefined, true).type === 'fit');
}
